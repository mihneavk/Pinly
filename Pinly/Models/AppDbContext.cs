...
namespace Pinly.Models
{
    public class AppDbContext : IdentityDbContext<ApplicationUser>
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            // Notifications
            builder.Entity<Notification>().HasOne(n => n.Sender).WithMany().HasForeignKey(n => n.SenderId).OnDelete(DeleteBehavior.Restrict);
            builder.Entity<Notification>().HasOne(n => n.Recipient).WithMany().HasForeignKey(n => n.RecipientId).OnDelete(DeleteBehavior.Cascade);
            builder.Entity<Notification>().HasOne(n => n.Group).WithMany().HasForeignKey(n => n.GroupId).OnDelete(DeleteBehavior.Cascade);
            // --- CHAT CONFIG ---
            // GroupMembership (Cheie compusa)
            builder.Entity<GroupMembership>().HasKey(gm => new { gm.GroupId, gm.ApplicationUserId });
            builder.Entity<GroupMembership>().HasOne(gm => gm.Group).WithMany(g => g.Memberships).HasForeignKey(gm => gm.GroupId).OnDelete(DeleteBehavior.Cascade);
            builder.Entity<GroupMembership>().HasOne(gm => gm.ApplicationUser).WithMany(u => u.Memberships).HasForeignKey(gm => gm.ApplicationUserId).OnDelete(DeleteBehavior.Restrict);

            // GroupMessage
            builder.Entity<GroupMessage>().HasOne(m => m.Sender).WithMany(u => u.GroupMessages).HasForeignKey(m => m.SenderId).OnDelete(DeleteBehavior.Restrict);

            // Group Moderator
            builder.Entity<Group>().HasOne(g => g.Moderator).WithMany(u => u.ModeratedGroups).HasForeignKey(g => g.ModeratorId).OnDelete(DeleteBehavior.SetNull);
        }
    }
}